...
PACELC-теорема:
Данные записываются на все узлы с задержкой до часа (асинхронная запись):
Так как корректный ответ может прийти в течение часа, то в системе присутствует доступность А. Система всегда стремится к консистентности С. Система будет PA/EC.

При сетевых сбоях, система может разделиться на 2 раздельных кластера:
Система будет PA/EL так как при разделении пропадет согласованность данных.

Система может не прислать корректный ответ или сбросить соединение:
В системе отсутствует доступность А. Система будет PC/EC, так как она может и сбросить соединение. 
